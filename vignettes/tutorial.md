...
all the resulting score tables, including the individual signature
scores and main ReMEA score (av.effect).

Output tables fall under the “\_combined” and “\_individual_dbs” slots.
The combined set represent the main ReMEA output, that is, the composite
...
For the analysis, ensure your proteomics data is a `data.table` or
`data.frame` (which will be converted internally into a `data.table`).
Proteins must annotated with their Uniprot Entry Name. This column name
can be specified with the `protein_id_col` argument. If not specified,
the first column with be used for the protein ID. Similarly, the numeric
column to be analysed (e.g. fold change) can be specified with the
`analysis_col` argument. If not specified, the second column will be
used for input.
...
markers of sensitivity to that perturbation. A negative score suggests
enrichment of resistance markers to that perturbation. The magnitude of
scores can differ between datasets (similar to other enrichment
methods). Accompanying p-values and overall ranking of scores should be
used to aid the interpretation of the returned results.
...
```
    ...
    ## | combined_ks_pvalue  | numeric   | The combined Kolmogorov-Smirnov p-value from the different signature selections. P-values are       |
    ## |                     |           | combined using Stouffer's method.                                                                   |
    ## | combined_bws_pvalue | numeric   | The combined BWS p-value from the different signature selections. P-values are combined using       |
    ## |                     |           | Stouffer's method.                                                                                  |
...
```
    ...
    ## | Column                    | Type      | Description                                                                                   |
    ## +---------------------------+-----------+-----------------------------------------------------------------------------------------------+
    ## | perturbagen               | character | The perturbation in question. For compounds, this includes the site-specific code from the    |
    ## |                           |           | GDSC2 drug annotations.                                                                       |
    ...
    ## | pvalue.ks                 | numeric   | The p-value from the Kolmogorov-Smirnov test comparing the resistance and sensitivity         |
    ## |                           |           | signatures.                                                                                   |
    ...
    ## |                           |           | delta_rank_median.                                                                            |
    ## | ks_qvalue                 | numeric   | Benjamini-Hochberg corrected Kolmogorov-Smirnov p-value. Adjusted within each                 |
    ## |                           |           | signature.type.                                                                               |
